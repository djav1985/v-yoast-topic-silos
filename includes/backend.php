...
function vyts_render_page_category_metabox( $post ) {
	...
	$saved_ids = is_array( $saved_ids ) ? array_filter( array_map( 'intval', $saved_ids ), function ( $id ) {
		return $id > 0;
	} ) : array();
	$saved_set = array_fill_keys( $saved_ids, true );
			...
			<li>
				<label>
					<input type="checkbox"
					       ...
					       <?php checked( isset( $saved_set[ (int) $category->term_id ] ) ); ?>>
...
function vyts_render_silo_metabox( $post ) {
	...
	if ( 'page' === $post->post_type ) {
		...
		if ( is_array( $saved ) ) {
			$category_ids = array_filter( array_map( 'intval', $saved ), function ( $id ) {
				return $id > 0;
			} );
		}
	} else {
	...
	$related_ids = array_values( array_unique( array_merge( $related_post_ids, $related_page_ids ) ) );
	$excluded_ids = array_merge( array( $post->ID ), $related_ids );
	...
	$has_related = ! empty( $related_ids );
	$has_other   = ! empty( $other_ids );
	...
	<?php if ( $has_other ) : ?>
		...
		<ul class="vyts-silo-list">
			<?php foreach ( $other_ids as $other_id ) : ?>
				<li>
					<button type="button"
					   class="vyts-copy-link"
